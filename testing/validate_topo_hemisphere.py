#!/usr/bin/env python3
"""
Validate the topocentric calculation works correctly in BOTH hemispheres.

Uses the same OEM points from artemis2_track.html, computes topocentric
RA/Dec using the JS-replica code and Astropy at two locations:
  1. SSO, Australia (lat=-31.28, lon=149.07) -- southern hemisphere
  2. UK (lat=53, lon=1) -- northern hemisphere

If both agree with Astropy to <1 arcsec, there is no hemisphere bug.
...
# ── JS-equivalent functions (same as validate_topo_vs_astropy.py) ──
...
observers = [
    ("SSO (south)", -31.2769, 149.0661, 1.164),
    ("UK (north)",   53.0,     1.0,     0.0),
]

test_indices = [0, 50, 100, 200, 500, 1000, 2000, 3000, len(oem_points)-1]

for name, lat, lon, alt in observers:
    print(f"\n{'='*80}")
    print(f"Observer: {name}  lat={lat}  lon={lon}  alt={alt} km")
    print(f"{'='*80}")
    hdr = f"{'UTC':>25s}  {'JS_RA':>10s} {'AP_RA':>10s} {'dRA_as':>8s}  {'JS_Dec':>10s} {'AP_Dec':>10s} {'dDec_as':>8s}  {'sep_as':>7s}"
    print(hdr)
    print('-' * len(hdr))
    ...
    for i in test_indices:
        ...
        ra_js, dec_js, _ = topoRADec_js(x, y, z, jd, lat, lon, alt)
        ra_ap, dec_ap, _ = topoRADec_astropy(x, y, z, utc_str, lat, lon, alt)
        ...
        print(f"{utc_str:>25s}  {ra_js:10.5f} {ra_ap:10.5f} {dra_sky:+8.3f}  {dec_js:10.5f} {dec_ap:10.5f} {ddec:+8.3f}  {sep:7.3f}")
